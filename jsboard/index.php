<?php
    ...
    {
        // 포럼 목록을 구합니다.
        $query = "SELECT TABLE_NAME AS table_name from information_schema.TABLES WHERE TABLE_SCHEMA = '{$db_info->db_database}' AND TABLE_NAME <> 'userdb' AND TABLE_NAME NOT LIKE '%_comm'";

        $forum_list_result = $oMigration->query($query);
        while ($forum_info = $oMigration->fetch($forum_list_result))
        {
            $forum_list[$forum_info->table_name] = $forum_info;
        }
        if (!$forum_list || !count($forum_list))
            $forum_list = array();

        // 포럼의 글 개수를 구합니다.
        foreach ($forum_list as $k => $forum_info) {
            $query = sprintf("SELECT COUNT(*) AS count FROM %s%s", $db_info->db_table_prefix, $forum_info->table_name);
            $result = $oMigration->query($query);
            $data = $oMigration->fetch($result);
            $forum_info->forum_posts = $data->count;
            $forum_list[$k] = $forum_info;
        }
    }
    ...
    if ($target_type)
    {
        ...
        else
        {
            switch ($target_type) {
                case 'member' :
                        $query = sprintf("SELECT COUNT(*) AS count FROM %s%s", $db_info->db_table_prefix, 'userdb');
                    break;
                case 'forum' :
                        $query = sprintf("SELECT COUNT(*) AS count FROM %s%s", $db_info->db_table_prefix, $forum_id);
                    break;
            }
            $result = $oMigration->query($query);
            $data = $oMigration->fetch($result);
...
<body>
    ...
    <form action="./index.php" method="post">
        <h3>1 단계. JSBoard 설치 경로 입력</h3>

        <ul>
            <li>
                JSBoard를 설치한 디렉토리의 경로를 입력 해주세요.

                <blockquote>
                예1) /home/아이디/public_html/JSBoard<br />
                예2) ../JSBoard
                </blockquote>
    ...
    ?>
        ...
        <input type="hidden" name="path" value="<?=$path?>" />
            ...
            <blockquote>JSBoard는 회원 정보와 포럼을 나누어 추출합니다.</blockquote>
            ...
            <ul>
                ...
                <li>
                    ...
                    <?php
                        ...
                        {
                            $cur_forum_id = $forum_info->table_name;
                            $title = sprintf('%s (%s)', $cur_forum_id, $forum_info->forum_posts);
    ...
    ?>
        ...
        <form action="./index.php" method="post">
            ...
            <blockquote>
                JSBoard 데이터 추출 파일을 다운로드 해주세요.<br />
