<?php 
    ...
    if ($target_type == 'member') {
        ...
        $query = "SELECT * FROM {$db_info->db_table_prefix}userdb ORDER BY user_id ASC " . $limit_query;
     ...
     **/
        ...
        // 첨부 파일 경로를 만듭니다.
        $attachment_path = sprintf('%s/data/%s/files/', $path, $forum_id);
        ...
        while ($document_info = mysql_fetch_object($document_result))
        {
            ...
            while ($comment_info = mysql_fetch_object($comment_result))
            {
                ...
                {
                    $comment_info->post_text = $comment_info->post_text;
                ...
                $comment_obj->user_id = $comment_info->username;
